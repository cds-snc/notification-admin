...
@pytest.fixture(scope="function")
def mock_get_non_govuser(mocker, api_user_active, user=None):
    if user is None:
        user = api_user_active
        user["email_address"] = "someuser@notonsafelist.com"
...
@pytest.fixture(scope="function")
def mock_get_user(mocker, api_user_active, user=None):
    if user is None:
        user = api_user_active
...
@pytest.fixture(scope="function")
def mock_get_login_events(mocker, api_user_active, user=None):
    if user is None:
        user = api_user_active
...
def mock_get_login_events_with_data(mocker, api_user_active, user=None):
    ...
    return mocker.patch("app.user_api_client.get_login_events_for_user", return_value=data)


@pytest.fixture(scope="function")
def mock_get_security_keys(mocker, api_user_active, user=None, keys=[]):
    if user is None:
        user = api_user_active
...
@pytest.fixture(scope="function")
def mock_get_organisation_user(mocker, api_user_active, user=None):
    if user is None:
        user = api_user_active
...
def mock_get_usage(mocker, service_one, fake_uuid):
    ...
    return mocker.patch("app.billing_api_client.get_service_usage", side_effect=_get_usage)
...
def mock_get_billable_units(mocker):
    ...
    return mocker.patch("app.billing_api_client.get_billable_units", side_effect=_get_usage)
...
def mock_get_safelist(mocker):
    ...
    return mocker.patch("app.service_api_client.get_safelist", side_effect=_get_safelist)
...
@pytest.fixture(scope="function")
def logged_in_client(client, active_user_with_permissions, mocker, service_one, mock_login):
