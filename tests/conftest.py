...
def mock_update_user_password(mocker, api_user_active):
    def _update(user_id, password, login_data={}):
...
def mock_get_job_with_api_key(mocker, api_user_active):
    def _get_job(service_id, job_id):
        return {"data": job_json(service_id, api_user_active, job_id=job_id, api_key={"name": JOB_API_KEY_NAME})}
...
def mock_validate_callback_url(mocker):
    return mocker.patch("app.main.validators.requests.post", return_value=Mock(content="a", status_code=200, headers={"a": "a"}))
