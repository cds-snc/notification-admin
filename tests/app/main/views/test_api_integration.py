...
def test_should_create_api_key_with_type_normal(
    ...
    key_name = "some_default_key_name_12"
    post = mocker.patch(
        "app.notify_client.api_key_api_client.ApiKeyApiClient.post",
        return_value={"data": {"data": fake_uuid, "key_name": key_name}},
    )
    ...
    assert page.select_one("span.api-key-key").text.strip() == ("{}-{}-{}".format(key_name, SERVICE_ONE_ID, fake_uuid))
...
def test_update_delivery_status_callback_details(
    ...
    mock_update_service_callback_api.assert_called_once_with(
        service_one["id"], url="https://test.url.com/", bearer_token="1234567890", user_id=fake_uuid, callback_api_id=fake_uuid
    )
