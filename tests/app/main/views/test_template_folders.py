...
def test_get_manage_folder_viewing_permissions_for_users(
    ...
    assert (
        "Test User" in page.findAll("label", {"for": "users_with_permission-0"})[0].text
    )
...
def test_delete_folder(
    client_request, service_one, mock_get_template_folders, mocker, parent_folder_id
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            active_user_view_permissions, marks=pytest.mark.xfail(raises=AssertionError)
        ),
        pytest.param(
            active_caseworking_user, marks=pytest.mark.xfail(raises=AssertionError)
        ),
    ],
)
...
def test_no_action_if_user_fills_in_ambiguous_fields(
    ...
    assert [ROOT_FOLDER_ID, FOLDER_TWO_ID, PARENT_FOLDER_ID,] == [
        radio["value"]
