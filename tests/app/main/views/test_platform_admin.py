...
@pytest.mark.parametrize(
    "endpoint, restricted, research_mode, displayed",
    [
...
def test_live_trial_services_toggle_including_from_test_key(
    partial_url_for, platform_admin_client, mock_get_detailed_services, endpoint, inc
):
...
@pytest.mark.parametrize(
    "endpoint, expected_big_numbers_single_plural, n_emails_sent, n_texts_sent, s_lang ",
    [
        (
            "main.live_services",
...
def test_platform_admin_list_complaints(platform_admin_client, mocker):
    complaint = {
        "id": str(uuid.uuid4()),
...
def test_clear_cache_requires_option(client_request, platform_admin_user, mocker):
    ...
    assert normalize_spaces(page.find("span", class_="error-message").text) == "Error: You need to choose an option"
    assert not redis.delete_cache_keys_by_pattern.called


def test_reports_page(platform_admin_client):
...
def test_usage_for_all_services_when_calls_api_and_download_data(platform_admin_client, mocker):
    ...
    response = platform_admin_client.post(
        url_for("main.usage_for_all_services"),
...
@pytest.mark.parametrize(
    ...
    [
        [
            {
                "date_created": "Tue, 01 Jan 2019 00:00:00 GMT",
                "service_id": SERVICE_ONE_ID,
                "service_name": "service one",
                "notification_type": "email",
                "count_sending": 191,
                "count_delivered": 0,
                "count_technical_failure": 0,
                "count_temporary_failure": 14,
                "count_permanent_failure": 0,
                "count_sent": 0,
            },
            {
                "date_created": "Tue, 01 Jan 2019 00:00:00 GMT",
...
def test_get_notifications_sent_by_service_calls_api_and_downloads_data(mocker, platform_admin_client, api_data):
    mocker.patch(
