...
jobs_1_failure = [job_with_failures_3]
...
def test_review_problem_emails_is_empty_when_no_probems(mocker, service_one, app_, client_request):
...
def test_review_problem_emails_shows_csvs_when_problem_emails_exist(
    mocker, service_one, app_, client_request: ClientRequest, jobs, expected_problem_list_count
):
    with freeze_time("2023-04-18T19:00:00+00:00"):
        threshold = app_.config["BR_DISPLAY_VOLUME_MINIMUM"]
        ...
        len(page.select_one("#within-24hrs").find_all("li")) == expected_problem_list_count
...
def test_review_problem_emails_shows_one_offs_when_problem_emails_exist(
    ...
    with freeze_time("2023-04-18T19:00:00+00:00"):
        ...
        # ensure the number of CSVs displayed on this page correspond to what is found in the jobs data
        assert len(page.select_one("#within-24hrs").find_all("li")) == expected_problem_list_count
...
def test_review_problem_emails_checks_problem_filter_checkbox(mocker, service_one, app_, client_request, jobs):
    ...
    # ensure the number of CSVs displayed on this page correspond to what is found in the jobs data
    assert "pe_filter=true" in page.select_one("#see-more").select_one("a")["href"]
    assert "status=permanent-failure" in page.select_one("#see-more").select_one("a")["href"]
