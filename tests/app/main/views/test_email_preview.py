...
@pytest.mark.parametrize(
    "query_args, result", [({}, True), ({"fip_banner_english": "false"}, "false")]
)
...
def test_displays_both_branding(client, mock_get_email_branding_with_both_brand_type):
    ...
    assert (
        page.select(
            "body > table:nth-of-type(2) table > tr:nth-of-type(1) > td:nth-of-type(2)"
        )[0]
        .get_text()
        .strip()
        == "Organisation text"
...
def test_displays_org_branding(client, mock_get_email_branding):
    ...
    assert (
        page.select(
            "body > table:nth-of-type(1) > tr:nth-of-type(1) > td:nth-of-type(1)"
        )[0]
        .get_text()
        .strip()
        == "Organisation text"
...
def test_displays_org_branding_with_banner(
    client, mock_get_email_branding_with_custom_logo_with_background_colour_brand_type
):
    ...
    assert response.status_code == 200
    mock_get_email_branding_with_custom_logo_with_background_colour_brand_type.assert_called_once_with(
        "1"
    )
