...
def test_attach_files_returns_404_when_ff_file_attachments_disabled(
    ...
    """Test that attach_files endpoint returns 404 when FF_FILE_ATTACHMENTS is disabled."""
    current_user.verified_phonenumber = True
    service_one["permissions"].append("upload_document")

    # Explicitly disable FF_FILE_ATTACHMENTS to test the disabled case
    with set_config(app_, "FF_FILE_ATTACHMENTS", False):
        # The _expected_status=404 parameter will cause the test to fail if the status is not 404
        client_request.post(
            ".attach_files",
            service_id=SERVICE_ONE_ID,
            template_id=fake_uuid,
...
class TestAnnualLimits:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (2, 2, "This message exceeds your annual limit."),
            # Daily = 0 (true zero), annual > 0 but < fragment_count → annual fragment-specific heading
            (0, 2, "This message exceeds your annual limit."),
        ],
    )
