...
def test_find_users_by_email_page_loads_correctly(client_request, platform_admin_user):
    ...
    assert document.h1.text.strip() == "Search for user"
...
def test_find_users_by_email_displays_users_found(
    ...
    assert any(
        element.text.strip() == "test@canada.ca"
    ...
    assert (
        document.find("a", {"class": "browse-list-link"}).text.strip()
        == "test@canada.ca"
    )
...
def test_user_information_page_does_not_show_archive_link_for_inactive_users(
    ...
    client.login(platform_admin_user)
    response = client.get(url_for("main.user_information", user_id=inactive_user["id"]))
    page = BeautifulSoup(response.data.decode("utf-8"), "html.parser")
...
def test_unblock_user_resets_failed_login_count(
    ...
    assert response.location == url_for(
        "main.user_information", user_id=api_user_active["id"], _external=True
    )
