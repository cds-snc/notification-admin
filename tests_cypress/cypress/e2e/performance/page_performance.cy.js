...
const URLS = {
    ...
    NOTIFICATIONS: '/services/{serviceId}/notifications/email?status=sending,delivered,failed',
};
...
const visitAndAssertPerformance = (url, label) => {
    ...
    cy.visit(url, { timeout: 10000 }).then(() => {
        ...
        expect(totalDuration, `${label} total load time`).to.be.lessThan(LOAD_TIME_THRESHOLD);
    });
};
...
describe('Page performance Tests - many templates/notifications', () => {
    ...
    before(() => {
        ...
        cy.get('body').contains(SERVICE_NAME).then(($element) => {
    ...
    it('Service dashboard should load in under 2 seconds', () => {
        visitAndAssertPerformance(URLS.DASHBOARD.replace('{serviceId}', serviceId), 'Dashboard');
    });

    it('Templates list should load in under 2 seconds', () => {
        visitAndAssertPerformance(URLS.TEMPLATES.replace('{serviceId}', serviceId), 'Templates list');
    });
    ...
    it('Template folder should load in under 2 seconds', () => {
        ...
        cy.get('a span').contains(FOLDER_NAME).then(($element) => {
            const href = $element.closest('a').attr('href');
    ...
    it('Notifications list should load in under 2 seconds', () => {
        visitAndAssertPerformance(URLS.NOTIFICATIONS.replace('{serviceId}', serviceId), 'Notifications list');
    });
});
