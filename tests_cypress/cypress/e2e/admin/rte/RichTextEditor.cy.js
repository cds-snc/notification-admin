import RichTextEditor, { FORMATTING_OPTIONS } from '../../../Notify/Admin/Components/RichTextEditor';

// Helper: reliably simulate Alt+F10 using native keyboard events
const pressAltF10 = () => {
    const keyEvent = { key: 'F10', code: 'F10', keyCode: 121, which: 121, altKey: true, bubbles: true };
    // focus and trigger keydown/keypress/keyup in sequence on the body
    return cy.get('body').focus().trigger('keydown', keyEvent).trigger('keypress', keyEvent).trigger('keyup', keyEvent);
};
...
describe('Toolbar accessibility tests', () => {
    ...
    const humanize = (key) => key.toLowerCase().replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
        ...
        Object.entries(FORMATTING_OPTIONS).forEach(([key, testId]) => {
            it(humanize(key), () => {
                ...
                button.should(($b) => {
                    ...
                    expect(text, `Label for ${humanize(key)} should contain "apply"`).to.match(/Apply/i);
                });
                button.should('have.attr', 'aria-pressed', 'false');

                // 2) type content and select it to make formatting apply-able
                RichTextEditor.Components.Editor().focus().type('{selectall}{del}');
                RichTextEditor.Components.Editor().focus().type('Button test: ' + testId);
                ...
                button.should(($b) => {
                    const text = $b.text().trim();
                    expect(text, `Label for ${humanize(key)} should contain "remove"`).to.match(/Remove/i);
                });
                button.should('have.attr', 'aria-pressed', 'true');

                // live region should announce the applied state; log its text for debugging
                RichTextEditor.Components.LiveRegion().should('contain.text', `applied`);
                ...
                button.should('have.attr', 'aria-pressed', 'false');
                RichTextEditor.Components.LiveRegion().should('contain.text', `removed`);
            });
        });
    });
    ...
    it("Toolbar has appropriate role and aria attributes", () => {
        ...
        RichTextEditor.Components.Toolbar().should('have.attr', 'aria-label').and('not.be.empty');
        RichTextEditor.Components.Toolbar().should('have.attr', 'aria-keyshortcuts', 'Alt+F10');
        RichTextEditor.Components.Toolbar().should('have.attr', 'aria-orientation', 'horizontal');
    });
    ...
    context("Toolbar keyboard shortcut", () => {
        ...
        it("Toolbar buttons can be accessed via arrow keys", () => {
            ...
            for (let i = total - 1; i > 0; i--) {
                ...
                RichTextEditor.Components.Toolbar().find('button').eq(i - 1).should('have.focus');
            }
        });
