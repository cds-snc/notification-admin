...
describe('Form autofocus', () => {
	const baseUrl = 'http://localhost:6012/_storybook?component=focus_management/';
	...
	context('Individual component pages', () => {
		...
		it('for select', () => {
			...
			AutoFocusPage.Actions.SubmitForm();
			AutoFocusPage.Components.ContactAutoFocusField().should('have.attr', 'autofocus');
		});

		it('for autocomplete', () => {
			cy.visit(`${baseUrl}autocomplete-form`);
			AutoFocusPage.Actions.SubmitForm();
			AutoFocusPage.Components.AutocompleteAutoFocusField().should('have.attr', 'autofocus');
		});

		it('for checkboxes', () => {
			cy.visit(`${baseUrl}checkboxes-form`);
			AutoFocusPage.Actions.SubmitForm();
			AutoFocusPage.Components.CheckboxesAutoFocusField().should('have.attr', 'autofocus');
		});

		it('for radios', () => {
			cy.visit(`${baseUrl}radios-form`);
			AutoFocusPage.Actions.SubmitForm();
			AutoFocusPage.Components.RadioAutoFocusField().should('have.attr', 'autofocus');
		});
	});
	...
	context('Full form page', () => {
		...
		it('for all field types', () => {
			...
			// ensure autofocus is on the next field with an error
			AutoFocusPage.Components.ContactAutoFocusField().should('have.attr', 'autofocus');
			AutoFocusPage.Components.EmailField().should('not.have.attr', 'autofocus');
			AutoFocusPage.Components.NameField().should('not.have.attr', 'autofocus');
			...
			// ensure autofocus is on the next field with an error
			AutoFocusPage.Components.CheckboxesAutoFocusField().should('have.attr', 'autofocus');
			AutoFocusPage.Components.AutocompleteField().should('not.have.attr', 'autofocus');
			AutoFocusPage.Components.ContactAutoFocusField().should('not.exist');
