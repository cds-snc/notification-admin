...
def delivery_status_callback(service_id):
    ...
    if form.validate_on_submit():
        ...
        response_time = "{:.2f} {}".format(
            g.callback_response_time, "milliseconds" if g.callback_response_time < 1 else "seconds")
        ...
        if delivery_status_callback and form.url.data:
            if delivery_status_callback.get("url") != form.url.data or form.bearer_token.data != dummy_bearer_token:
                ...
                if request.form.get("button_pressed") == "test_response_time":
                    flash(_l("Your service '{}' responded in {}").format(
                        form.url.data,
                        response_time,
                    ), "default_with_tick")
                    return redirect(url_for("main.delivery_status_callback", service_id=service_id))

                flash(_l("Callback to '{}' saved. Your service responded in '{}'").format(
                    form.url.data.split('https://')[1],
                    response_time,
                ), "default_with_tick")
        ...
        # Create a new callback
            ...
            flash(_l("Callback to '{}' created. Your service responded in '{}'").format(
                form.url.data.split('https://')[1],
                response_time,
            ), "default_with_tick")
    ...
    return render_template(
        "views/api/callbacks/delivery-status-callback.html",
        has_callback_config=not delivery_status_callback is None,
        back_link=back_link,
        hint_text=url_hint_txt,
        form=form,
    )
