...
from app.main.views.reports import get_report_footer_partial
...
def get_notifications(service_id, message_type, status_override=None):
    ...
    reports_partials = get_report_footer_partial(service_id=service_id)
    return {
        "service_data_retention_days": service_data_retention_days,
...
def get_job_partials(job, template):
    ...
    reports_partials = get_report_footer_partial(service_id=current_service.id)
    return {
        "counts": counts,
