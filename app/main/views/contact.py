...
def contact():
    current_step = request.args.get('current_step') or session.get("contact_form_step", "identity")
    ...
    if form.validate_on_submit():
        ...
        if len(candidates) == 1:
            ...
            form_obj = [e for e in candidates if form.support_type.data in e["support_types"]][0]
        form = form_obj["form"](data=session.get("contact_form", {}))
    ...
    return render_template(
        ...
        step=form_obj.get("step", None),
        total_steps=form_obj.get("total_steps", None)
    )
