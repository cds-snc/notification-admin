  ...
  Modules.ApiKey = function() {
    ...
    const states = {
      'keyVisible': (key, thing) => `
        ...
        <input type='button' class='absolute bottom-2 active:top-auto button button-secondary' value='${window.polyglot.t('copy')} ${thing} ${window.polyglot.t('to_clipboard')}' />
      `,
      'keyCopied': thing => `
        <span class="api-key-key">${window.polyglot.t('copied_to_clipboard')}</span>
        <input type='button' class='absolute bottom-2 active:top-auto button button-secondary' value='${window.polyglot.t('show')} ${thing}' />
      `
    };
